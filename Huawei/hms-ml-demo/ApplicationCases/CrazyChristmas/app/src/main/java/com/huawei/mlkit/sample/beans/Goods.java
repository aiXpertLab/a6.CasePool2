...
public class Goods {
    ...
    public boolean hit(Sled shoopCart) {
        if ((x + goodsWidth / 2) > (shoopCart.x - shoopCart.sledWidth / 2) && (x - goodsWidth / 2) < (shoopCart.x + shoopCart.sledWidth)) {
            if ((y + goodsHeight / 2) > (shoopCart.y - shoopCart.sledHeight / 2) && (y - goodsHeight / 2 < shoopCart.y + shoopCart.sledHeight / 2)) {
                image = images[random.nextInt(goods.length)];
                x = goodsWidth / 2 + random.nextInt(screenWidth - goodsWidth);
                y = -goodsHeight / 2;
                return true;
